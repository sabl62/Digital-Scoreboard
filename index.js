...
function updateDisplays() {
  ...
  elements.timeDisplay.style.color =
    gameState.timeRemaining <= 10 ? "var(--danger)" : "var(--accent)";
...
function startStopClock() {
  if (gameState.clockRunning) {
    ...
    elements.clockControl.innerHTML =
      '<i class="fas fa-pause"></i> PAUSE CLOCK';
...
document.querySelectorAll("[data-points]").forEach((btn) => {
  btn.onclick = () => {
    ...
    if (team === "team1")
      gameState.team1Score = Math.max(0, gameState.team1Score + pts);
...
const handleManualTime = () => {
  gameState.timeRemaining =
    parseInt(elements.minutesInput.value) * 60 +
    parseInt(elements.secondsInput.value);
